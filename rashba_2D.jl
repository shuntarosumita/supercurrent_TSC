...
function calculate_spectrum_pbc(q::Vector{Float64}, param::Vector{Float64})
    ...
    filename = @sprintf(
        "data/spec2Dp_%.3ft1_%.3ft2_%.3fa1_%.3fa2_%.3fm_%.3fdd_%.3fdp_%.3fqx_%.3fqy.jld2",
        param..., q...
    )
...
function plot_spectrum_pbc(q::Vector{Float64}, param::Vector{Float64})
    ...
    filename = @sprintf(
        "spec2Dp_%.3ft1_%.3ft2_%.3fa1_%.3fa2_%.3fm_%.3fdd_%.3fdp_%.3fqx_%.3fqy",
        param..., q...
    )
    ...
    # gnuplot
    range = "[pi*11/32:pi*18/32]"
    tics = "('1.1' 1.1, '1.7' 1.7)"
...
function calculate_Berry_phase_mesh(q::Vector{Float64}, param::Vector{Float64})
    ...
    isdir("data") || mkdir("data")
    filename = @sprintf(
        "data/berry2D_%.3ft1_%.3ft2_%.3fa1_%.3fa2_%.3fm_%.3fdd_%.3fdp_%.3fqx_%.3fqy.jld2",
        param..., q...
    )
    datafile = File(format"JLD2", filename)
...
function plot_Berry_phase_mesh(q::Vector{Float64}, param::Vector{Float64})
    # get data of eigenvalues
    filename = @sprintf(
        "berry2D_%.3ft1_%.3ft2_%.3fa1_%.3fa2_%.3fm_%.3fdd_%.3fdp_%.3fqx_%.3fqy",
        param..., q...
    )
    isfile("data/" * filename * ".jld2") || error("no data exists")
    data = load("data/" * filename * ".jld2")
...
#     # plot eigenvalues
#     plt = heatmap(data["ks"], data["ks"], permutedims(data["γmesh"], (2, 1)),
#         xlims=(0, π), ylims=(0, π), clims=(0, 1), c=cgrad(:coolwarm),
#         size=(480, 400), left_margin=0mm, right_margin=5mm, top_margin=3mm,
#         tickfont=font(14), guidefont=font(18),
#         xticks=([0, π], ["0", "\\pi"]), yticks=([0, π], ["0", "\\pi"]),
#         xlabel=L"k_x", ylabel=L"k_y", aspect_ratio=:equal, dpi=300
...
#     plot!(plt, [i1 * π / nmesh for i1 in 1:nmesh], seriestype=:hline,
#         linewidth=0.5, linestyle=:dot, linecolor=:black, label=:none
#     )
#     plot!(plt, [i1 * π / nmesh for i1 in 1:nmesh], seriestype=:vline,
#         linewidth=0.5, linestyle=:dot, linecolor=:black, label=:none
#     )
